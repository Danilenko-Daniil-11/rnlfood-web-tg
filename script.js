    ...
    try {
        ...
        data.forEach(achievement => {
            ...
            achievementElement.innerHTML = `
                ...
                ${isUnlocked ? `<div class="achievement-points">+${achievement.points_reward} баллов</div>` : ''}
            `;

            if (isUnlocked) {
                achievementElement.title = `Разблокировано: ${new Date(achievement.unlocked_at).toLocaleDateString('ru-RU')}`;
            }
        ...
        // Fallback к статическим достижениям
        loadStaticAchievements(container);
    }
}

function loadStaticAchievements(container) {
...
// Функция показа отзывов о блюде
async function showMealReviews(mealId, mealName) {
    try {
        const data = await apiRequest(`/api/meals/${mealId}/reviews`);
        ...
        if (data.reviews && data.reviews.length > 0) {
            ...
            reviewsHtml = '<div class="no-reviews">Пока нет отзывов</div>';
        }

        modal.innerHTML = `
            <div class="modal-content">
                <div class="modal-header">
                    <h3>Отзывы о "${mealName}"</h3>
                    <button class="modal-close btn-particle" onclick="closeModal('reviews-modal')">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="average-rating">
                        <h4>Средний рейтинг: ${data.average_rating.toFixed(1)} ⭐</h4>
                        <p>${data.total_reviews} отзывов</p>
                    </div>
                    ...
                    ${currentUser ? `
                        <div class="add-review-section">
                            ...
                            <div class="rating-input">
                                ...
                                <div class="star-rating-input" id="review-rating">
                                    ${[1,2,3,4,5].map(i => `<i class="far fa-star" data-rating="${i}"></i>`).join('')}
                                </div>
                            </div>
                            <textarea id="review-comment" placeholder="Ваш отзыв (необязательно)" rows="3"></textarea>
                            <button class="btn-primary btn-particle" onclick="submitReview('${mealId}', '${mealName}')">
                                Отправить отзыв
                            </button>
                        </div>
        ...
        if (currentUser) {
            const stars = modal.querySelectorAll('#review-rating i');
            stars.forEach(star => {
...
async function submitReview(mealId, mealName) {
    const rating = document.querySelector('#reviews-modal').dataset.selectedRating;
    ...
    // Находим order_item_id для этого блюда (последний завершенный заказ пользователя)
    try {
        const ordersData = await apiRequest('/api/orders/history?limit=10');
        let orderItemId = null;

        for (const order of ordersData.orders || []) {
            if (order.status === 'completed' && order.items) {
                const item = order.items.find(item => item.name === mealName);
                if (item) {
                    // Получаем order_item_id из деталей заказа
                    const orderDetails = await apiRequest(`/api/orders/${order.id}`);
                    if (orderDetails.items) {
                        const detailItem = orderDetails.items.find(i => i.name === mealName);
        ...
        if (!orderItemId) {
            showNotification('Вы должны сначала заказать это блюдо, чтобы оставить отзыв', 'error');
            return;
        }
        ...
        // Обновляем отображение рейтинга в меню
        updateMealRating(mealId);

    } catch (error) {
        console.error('Ошибка отправки отзыва:', error);
        showNotification(error.message || 'Ошибка отправки отзыва', 'error');
    }
}

// Функция обновления рейтинга блюда
async function updateMealRating(mealId) {
    try {
        const data = await apiRequest(`/api/meals/${mealId}/reviews`);
        const mealCard = document.querySelector(`.item-card[data-product="${mealId}"]`);
        if (mealCard) {
            const ratingElement = mealCard.querySelector('.rating');
            if (ratingElement && data.average_rating > 0) {
                ratingElement.innerHTML = generateStarRating(data.average_rating.toFixed(1));
            }
        }
    } catch (error) {
...
async function loadLoyaltyInfo() {
    ...
    try {
        const data = await apiRequest('/api/loyalty');

        // Добавляем информацию о баллах в профиль
        const profileCard = document.querySelector('.profile-card:nth-child(1)');
        if (profileCard) {
            const cardContent = profileCard.querySelector('.card-content');
            const loyaltyHtml = `
                <div class="info-item">
                    <span class="info-label">Баллы лояльности:</span>
                    <span class="info-value">${data.points || 0} баллов</span>
                </div>
            `;
            cardContent.insertAdjacentHTML('beforeend', loyaltyHtml);
        }

        // Показываем последние транзакции
        if (data.recent_transactions && data.recent_transactions.length > 0) {
            const transactionsHtml = data.recent_transactions.slice(0, 3).map(t => `
                <div class="loyalty-transaction">
                    <span>${t.reason}</span>
                    <span class="${t.points > 0 ? 'positive' : 'negative'}">${t.points > 0 ? '+' : ''}${t.points} баллов</span>
                </div>
            `).join('');

            const profileCard = document.querySelector('.profile-card:nth-child(2)');
            if (profileCard) {
                const cardContent = profileCard.querySelector('.card-content');
                cardContent.insertAdjacentHTML('afterbegin', `
                    <div class="loyalty-section">
                        <h4>Недавние баллы лояльности:</h4>
                        ${transactionsHtml}
                    </div>
                `);
            }
        ...
        console.error('Ошибка загрузки программы лояльности:', error);
    }
}
...
function toggleParentMode() {
    ...
    if (isParentMode) {
        ...
        // Добавляем родительские функции
        addParentControls();
        ...
        // Убираем родительские функции
        removeParentControls();
    }
}
...
async function loadParentChildren() {
    ...
    try {
        const children = await apiRequest('/api/parent/children');

        if (children.length > 0) {
            // Создаем панель управления детьми
            const parentPanel = document.createElement('div');
            parentPanel.id = 'parent-panel';
            parentPanel.className = 'parent-panel';
            parentPanel.innerHTML = `
                <div class="parent-panel-header">
                    <h3><i class="fas fa-user-shield"></i> Управление детьми</h3>
                    <button class="btn-close" onclick="hideParentPanel()">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                <div class="parent-children-list">
                    ${children.map(child => `
                        <div class="parent-child-item" data-username="${child.username}">
                            <div class="child-info">
                                <span class="child-name">${child.full_name}</span>
                                <span class="child-class">${child.class_name}</span>
                                <span class="child-balance">Баланс: ${child.balance} ₴</span>
                            </div>
                            <div class="child-actions">
                                <button class="btn-secondary btn-particle" onclick="viewChildOrders('${child.username}')">
                                    <i class="fas fa-list"></i> Заказы
                                </button>
                                <button class="btn-primary btn-particle" onclick="topupChildBalance('${child.username}')">
            ...
            slideIn(parentPanel, 'down');
        } else {
            showNotification('У вас нет связанных детей. Используйте функцию "Связать ребенка"', 'info');
        }

    } catch (error) {
        console.error('Ошибка загрузки детей:', error);
        showNotification('Ошибка загрузки информации о детях', 'error');
    }
}
...
async function viewChildOrders(childUsername) {
    try {
        const orders = await apiRequest(`/api/child/orders/${childUsername}`);

        const modal = document.createElement('div');
        modal.className = 'modal';
        modal.id = 'child-orders-modal';

        let ordersHtml = '';
        if (orders.length > 0) {
            ordersHtml = orders.map(order => `
                <div class="child-order-item">
                    <div class="order-header">
                    ...
                    <div class="order-total">
                        <strong>Итого: ${order.final_amount} ₴</strong>
                    </div>
                </div>
            `).join('');
        } else {
            ordersHtml = '<div class="no-orders">Заказов пока нет</div>';
        }

        modal.innerHTML = `
            <div class="modal-content large-modal">
                <div class="modal-header">
                    <h3>Заказы ребенка: ${childUsername}</h3>
                    <button class="modal-close btn-particle" onclick="closeModal('child-orders-modal')">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                <div class="modal-body">
        ...
        document.body.appendChild(modal);
        openModal('child-orders-modal');

    } catch (error) {
        console.error('Ошибка загрузки заказов ребенка:', error);
        showNotification('Ошибка загрузки заказов ребенка', 'error');
    }
}

function topupChildBalance(childUsername) {
    showNotification('Функция пополнения баланса ребенка скоро будет доступна', 'info');
}

function addParentControls() {
    // Добавляем кнопку связи с ребенком в профиль
    const profileActions = document.querySelector('.profile-actions');
    if (profileActions) {
        const linkChildBtn = document.createElement('div');
        linkChildBtn.className = 'btn-contain';
        linkChildBtn.innerHTML = `
            <button class="btn-secondary btn-particle" onclick="showLinkChildModal()">
                <i class="fas fa-user-plus"></i>
                Связать ребенка
            </button>
        `;
        profileActions.appendChild(linkChildBtn);
    }
}

function removeParentControls() {
    // Убираем родительские элементы
    const parentPanel = document.getElementById('parent-panel');
    if (parentPanel) {
        parentPanel.remove();
    }

    const linkChildBtn = document.querySelector('.profile-actions .btn-contain:last-child');
    if (linkChildBtn && linkChildBtn.textContent.includes('Связать ребенка')) {
        linkChildBtn.remove();
    }
}
...
function showLinkChildModal() {
    ...
    modal.innerHTML = `
        <div class="modal-content">
            <div class="modal-header">
                <h3>Связать ребенка</h3>
                <button class="modal-close btn-particle" onclick="closeModal('link-child-modal')">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="modal-body">
                <div class="input-group">
                    <input type="text" id="child-username" placeholder=" " required>
                    <label for="child-username">Логин ребенка</label>
                </div>
                <p class="modal-info">Введите логин ребенка, чтобы связать аккаунты</p>
            </div>
            <div class="modal-footer">
                <div class="btn-contain" style="display: inline-block;">
                    <button class="btn-secondary btn-particle" onclick="closeModal('link-child-modal')">Отмена</button>
                </div>
                <div class="btn-contain" style="display: inline-block;">
                    <button class="btn-primary btn-particle" onclick="linkChild()">Связать</button>
                </div>
            </div>
        </div>
    `;
...
async function linkChild() {
    const childUsername = document.getElementById('child-username').value.trim();

    if (!childUsername) {
        showNotification('Введите логин ребенка', 'error');
        return;
    }

    try {
        await apiRequest('/api/parent/link', {
            method: 'POST',
            body: { child_username: childUsername }
        });

        showNotification('Ребенок успешно связан!', 'success');
        closeModal('link-child-modal');

        // Перезагружаем информацию о детях
        loadParentChildren();

    } catch (error) {
        console.error('Ошибка связи с ребенком:', error);
        showNotification(error.message || 'Ошибка связи с ребенком', 'error');
    }
}
    ...
    prompt.innerHTML = `
        <div class="push-prompt-content">
            ...
            <div class="push-prompt-text">
                ...
                <p>Получайте уведомления о статусе заказов и специальных предложениях</p>
            </div>
            <div class="push-prompt-buttons">
                <button class="btn-secondary" onclick="hidePushPrompt()">Позже</button>
                <button class="btn-primary" onclick="subscribeToPush()">Включить</button>
            </div>
        </div>
    `;
...
// Добавляем кнопку отзывов к карточкам блюд
function addReviewButtonsToMenu() {
    ...
    itemCards.forEach(card => {
        ...
        if (!existingBtn) {
            ...
            if (actionsDiv) {
                ...
                reviewBtn.className = 'review-btn btn-particle';
                reviewBtn.innerHTML = '<i class="fas fa-star"></i>';
                reviewBtn.title = 'Посмотреть отзывы';
                reviewBtn.onclick = function(e) {
                    e.stopPropagation();
                    const mealName = card.querySelector('.item-name').textContent;
                    const mealId = card.querySelector('.favorite-btn').dataset.product;
                    showMealReviews(mealId, mealName);
                };
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Добавляем кнопки отзывов при загрузке меню
    setTimeout(() => {
        addReviewButtonsToMenu();
    }, 2000);

    // Загружаем информацию о лояльности при входе в профиль
