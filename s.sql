-- ===========================================
-- RNL FOOD - УЛУЧШЕННАЯ СХЕМА БАЗЫ ДАННЫХ
-- Версия 2.0 - 10-кратное улучшение
-- ===========================================
...
DO $$ BEGIN
    CREATE TYPE notification_type AS ENUM ('info', 'warning', 'success', 'error', 'achievement');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
DO $$ BEGIN
    CREATE TYPE meal_difficulty AS ENUM ('easy', 'medium', 'hard');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
    CREATE TYPE balance_transaction_method AS ENUM ('crypto', 'card', 'cash', 'transfer', 'reward');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
